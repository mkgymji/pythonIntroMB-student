...
\begin{tcolorbox}[colback=code-bg, colframe=gray!40, fonttitle=\small\bfseries,
                  title=Jak použít REPL v Thonny]
  \small
  REPL je interaktivní panel v~dolní části Thonny (záložka \textbf{Shell}).
  Funguje jako kalkulačka — napíšeš výraz, stiskneš Enter, hned vidíš výsledek.\\[4pt]
  \textbf{Pro tuto úlohu:}
  \begin{enumerate}[leftmargin=*, itemsep=1pt]
    \item Ujisti se, že Thonny je připojeno k~micro:bitu (dole vidíš \texttt{MicroPython (BBC micro:bit)}).
    \item V~panelu Shell napiš: \pyinline{zprava = "Hello, Microbit!"} a stiskni Enter.
    \item Pak zadávej jednotlivé příkazy z~kódu výše — vždy jeden řádek, Enter, výsledek.
  \end{enumerate}
\end{tcolorbox}
...
\begin{tcolorbox}[colback=code-bg, colframe=gray!40,
  ...
  \begin{itemize}[leftmargin=*]
    ...
    \item \pyinline{if msg is not None:} — \pyinline{is not} testuje, zda hodnota \textbf{není}
          \pyinline{None}. Vždy používej \pyinline{is not None}, ne \pyinline{!=}.
  \end{itemize}
\end{tcolorbox}
