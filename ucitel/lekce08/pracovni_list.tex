...
Vezmi jeden z~těchto programů z~předchozích lekcí a přepiš ho pomocí funkcí:

\begin{itemize}
  \item \textbf{Teploměr} (lekce~6) — funkce \pyinline{zobraz\_stav(teplota)} vrátí
        správný obrázek podle pásma
  \item \textbf{Věštírna} (lekce~5) — funkce \pyinline{vestein()} vrátí náhodnou odpověď
  \item \textbf{Jukebox} (lekce~7) — funkce \pyinline{zahraj(melodie)} přehraje melodii
        a zobrazí animaci
\end{itemize}
...
def generuj_hody(pocet, strany=6):   # strany=6 je vychozi hodnota parametru
    vysledky = []
...
\begin{enumerate}
  ...
  \item \textbf{Výzva:} Napiš funkci \pyinline{prumer(seznam)}, která vrátí
        průměr čísel ze seznamu.
        \textit{(\pyinline{sum(seznam)} je vestavěná funkce Pythonu — sečte
        všechna čísla v seznamu. \pyinline{len(seznam)} znáš z lekce~07.)}
